...
var (
	pklPackage     = "package://pkg.pkl-lang.org/github.com/avarei/function-pkl/crossplane-example@0.1.10"
	pklCorePackage = "package://pkg.pkl-lang.org/github.com/avarei/function-pkl/crossplane@0.0.25"
		...
		"SingleResource-Inline": {
			...
			args: args{
			   ...
			   resource = (observedCompositeResource) {
			     status {
			       when (cmOneObj?.metadata?.namespace != null) {
			         someStatus = "I observed cm-one's namespace. it is \(cmOneObj.metadata.namespace)"
			       }
			     }
			   }
			   connectionDetails {
			...
			args: args{
				...
				req: &fnv1beta1.RunFunctionRequest{
					...
					Input: resource.MustStructObject(&v1beta1.Pkl{
						Spec: v1beta1.PklSpec{
							CRDs: DefaultCRDs(),
							Full: &v1beta1.PklFileRef{
								Name: "all-in-one",
								Type: "uri",
								Uri:  pklPackage + "#/all-in-one.pkl",
							},
						},
					}),
